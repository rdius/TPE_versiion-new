...
species pollutant_grid skills:[moving] {			
	...
	reflex separation when: apply_separation {
			...
			loop pollutant over: (pollutant_grid at_distance (minimal_distance))  {
				acc <- acc - ((location of pollutant) - location);
			}  
...
//		}
		...
		monitor "Quantite de PM" value: pollutant_grid count (each.type = "PM");
		monitor "Quantite de CO" value: pollutant_grid count (each.type = "CO");
		monitor "Quantite de SO2" value: pollutant_grid count (each.type = "SO2");
		monitor "Quantite de NOx" value: pollutant_grid count (each.type = "NOx");
		...
		display chart_display refresh_every: 15 {		
			...
			chart "Gas Status" type: series 
			{
				...
				//data "NO" value: pollutant_grid count (each.type = "NO") style: line color: # blue;
//					data "PM" value:pollutant_grid  count (each.type = "PM") style: line color: # green;
//					data "CO" value: pollutant_grid count (each.type = "CO") style: line color: # orange;
//					data "SO2" value: pollutant_grid count (each.type = "SO2") style: line color: # red;
//					data "NOX" value: pollutant_grid count (each.type = "NOX") style: line color: # blue;
			}
